...
class PWL(object):
    ...
    def run_PWL(self):
        ...
        def find_slope(y_dif):
            p = [0,] + [2**i for i in list(range(-self.shift_bit,self.shift_bit))]
    ...
    def gen_verilog(self, filename = "auto_gen.v"):
        ...
        for bound,slope,bound_n in zip(b,[0,] + s[:-1], [b[0],]+b[:-1]):
            ...
            if slope == 0:
                ...
                try:
                    f.write("\t\tslope = 16'h{:s};\n".format(to_hex(int(-np.log2(slope)))))
                    f.write("\t\tzero = {:d};\n".format(0))
                except:
                    f.write("\t\tslope = 16'h{:d};\n".format(0))
                    f.write("\t\tzero = {:d};\n".format(1))
            f.write("\t\tx_delta = x-16'h{:s};\n".format(to_hex(int(bound_n*(2**8)))))
            f.write("\tend else ")
